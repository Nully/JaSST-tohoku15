...
- 一人前になるまではどうやって教育するのか？

一人前になるまではコードレビューで教育していく。

また、働き方については、それ自体に振り返り時間を設けてKPTなりで振り返りを行う

- レビューは遠隔地で行っているのか？

基本はすべてGithub上で行う。
ただ、文字だけでは意図が伝わらない場合はTV会議などで直接話しをする。
...
質問とかは予めチャットツールだったり、直接質問してしまい、問題を解決しておく。
その上で、PRをだしてレビューを行う。

## ワークレビュー

1週間毎にKとPをだし、一人前になった人とTを洗い出す。
それを繰り返し、人を成長させる

```
機能を作ることが仕事の終了ではなく、リファクタリングが発生したらそれは仕事が終わっていないものと同義
```


## チャットの悪いところ

顔がわからないからremottyで顔を常に移しておきながら対話をする
